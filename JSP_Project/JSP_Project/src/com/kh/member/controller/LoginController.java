...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1) 전달값에 한글이 있을 경우 인코딩 처리 해야함(POST방식일때만)
		...
		/*
		 * 응답 페이지에 전달한 값이 있을 경우 값을 어딘가에 담아야함(담아줄 수 있는 Servlet Scope의 내장객체 4종류)
		 * 
		 ...
		 * 4) page		  : jsp에서만 꺼내 쓸 수 있음
		 * 
		 * => session과 request를 주로 사용
		 * 
		 * 공통적으로 데이터를 담을 때 : .setAttribute("키", "벨류");
		 * 				 꺼낼 때 : .getAttrribute("키");
		 * 				 지울 때 : .removeAttrribute("키");
		 */
		if(loginUser == null) { // 로그인 실패 => 에러페이지 응답
			...
			// 로그인한 회원의 정보를 로그아웃하기 전까지 계속 가져다 쓸 것이기 때문에 session에 데이터를 담기
			
			// Servlet에서 JSP내장객체인 SESSOIN에 접근하고자 한다면 우선 session 객체를 얻어와야함
			...
			// 1. 포워딩 방식 응답뷰 출력하기
			//	  선택된 jsp가 보여질 뿐 url에는 여전히 현재 이 서블릿에 대한 매핑값이 남아있을 것임
			//	  localhost:8082/jspproject/login.me
			//	  url에 내용이 그대로 남아있는 상태에서 새로고침(f5)을 누르게되면 계속 로그인 요청을 보내게 됨
