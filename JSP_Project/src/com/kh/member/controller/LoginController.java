...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * - request : 서버로 요청할 때의 정보들이 담겨있음(요청 시 전달한 값, 전송 방식 등)
		...
		// 1) 전달값에 한글이 있을 경우 인코딩 처리해야 함(POST 방식일 때만)
		...
		// 4) 처리된 결과를 가지고 사용자가 보게 될 화면(view)를 지정 후 포워딩 or url 재요청
		
		/*
		 * 응답 페이지에 전달한 값이 있을 경우 값을 어딘가에 담아야 함(담아줄 수 있는 Servlet Scope의 내장 객체 4종류)
		 * 
		 * 1) application : application에 담은 데이터는 웹 애플리케이션 전역에서 다 꺼내 쓸 수 있다.
		 * 2) session : session에 담은 데이터는 모든 jsp와 servlet에서 꺼내 쓸 수 있다.
		 *              한번 담은 데이터는 내가 직접 지우기 전까지, 서버가 멈추기 전까지, 브라우저가 종료되기 전까지 접근해서 꺼내 쓸 수 있다.
		...
		if(loginUser == null) { // 로그인 실패 => 에러 페이지 응답
		...
		} else { // 로그인 성공 => index 페이지(메인 페이지) 응답
			
			// 로그인한 회원의 정보를 로그아웃하기 전까지 계속 가져다가 쓸 것이기 때문에 session에 데이터 담기
			
			// Servlet에서 JSP 내장 객체인 SESSION에 접근하고자 한다면 우선 session 객체를 얻어와야 함
			...
			//    url에 내용이 그대로 남아있는 상태에서 새로고침 누르게 되면 계속 로그인 요청을 보내게 될 것
			
			// 2. url 재요청 방식(sendRedirect 방식)
			//    localhost:8081/jspproject라는 url로 재요청 보냄
			response.sendRedirect(request.getContextPath()); // 로그인 시에는 redirect 방식이 사용된다.
		}
	}
