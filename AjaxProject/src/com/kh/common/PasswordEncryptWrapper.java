...
public class PasswordEncryptWrapper extends HttpServletRequestWrapper {
	...
	// getParameter() 오버라이딩
	// 매개 변수로 넘어온 password값만 찾아서 암호화 시켜줄 예정
	...
	public String getParameter(String name) {
		...
		// 매개 변수로 전달받은 name 변수의 값이 userPwd일 때 암호화 작업 수행하기
	...
	public String getSHA512(String val) {
		...
		// 암호화는 bit 연산하고, bit 연산한 결과를 byte[]에 담아서 보관
		byte[] bytes = val.getBytes(Charset.forName("UTF-8"));
		md.update(bytes);
		
		// bit 연산하고 그 결괏값은 byte[] 배열이므로 String 형태로 값을 변환
