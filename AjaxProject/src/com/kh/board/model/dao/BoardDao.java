...
public class BoardDao {
	...
	public ArrayList<Board> selectList(Connection conn, PageInfo pi) {
		...
		try {
			 ...
			 * currentPage 1을 요청 시 -> 1 ~ 10
			 * currentPage 2를 요청 시 -> 11 ~ 20
			 * currentPage n을 요청 시 -> 시작 값 : (currentPage - 1) * boardLimit + 1
			 *                          마지막 값 :  시작 값 + boardLimit - 1
			 */
	...
	public ArrayList<Board> selectThumbnailList(Connection conn) {
		...
		try {
			...
			while (rset.next()) {
				...
				b.setTilteImg(rset.getString("TITLE_IMG"));
	...
	public ArrayList<Board> searchList(Connection conn, String searchType, String keyword) {
		...
		// 지금은 serchType값을 하드코딩해두었지만 사용자의 요청 방법에 맞게끔 변경도 가능해야 한다.
		...
		sql = sql.replace("@", type); // board-mapper의 @ 부분을 내가 원하는 검색 조건으로 바꿔주기
